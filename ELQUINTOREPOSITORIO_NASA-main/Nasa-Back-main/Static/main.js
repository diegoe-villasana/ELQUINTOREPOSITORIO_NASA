document.addEventListener('DOMContentLoaded', function() {
    ...
    if (carouselSlide) {
        ...
        // Ajustar el ancho del slide dinámicamente y usar flex para que cada "slide" ocupe 100% del viewport
        carouselSlide.style.width = `${numImages * 100}%`;
        carouselSlide.style.display = 'flex';
        carouselSlide.style.flexWrap = 'nowrap';

        // Si las imágenes están envueltas en <figure> u otros contenedores, tratamos a cada hijo como un slide
        const slides = Array.from(carouselSlide.children);
        slides.forEach(slide => {
            slide.style.flex = `0 0 ${100 / numImages}%`;
            slide.style.maxWidth = `${100 / numImages}%`;
        ...
        // Asegurar que las imágenes llenen su contenedor y mantengan aspect ratio
        images.forEach(img => {
            img.style.width = '100%';
